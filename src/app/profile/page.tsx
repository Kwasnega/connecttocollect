...
export default function ProfilePage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    setIsUpdating(true);
    try {
      // Identity Upsert Protocol: Uses merge:true to ensure node existence without permission conflicts
      ...
      toast({ 
        ...
        description: "Your official identity records have been saved." 
      });
    } catch (error: any) {
      toast({ 
        variant: "destructive", 
        title: "Update Failed", 
        description: "Technical error during synchronization. Please check your connection." 
      });
    } finally {
      setIsUpdating(false);
    }
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <Link href="/dashboard">
              <Button variant="ghost" className="mb-4 text-[10px] font-black uppercase tracking-widest text-primary hover:bg-white p-0 h-auto">
                <ArrowLeft className="w-4 h-4 mr-2" /> Command Hub
              </Button>
            </Link>
